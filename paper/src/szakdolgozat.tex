  ...
  \begin{figure}[!htb]
    ...
    \minipage{0.5\textwidth}
      ...
      \lstinputlisting[language=JavaScript,
                       firstnumber=1,autogobble=true,
                       linerange={11-12,25-28,31-31,33-33, 35-35}]
      {paper/examples/triangle/triangle.js}
    \endminipage
    \hfill
    \minipage{0.5\textwidth}
...
alakzatot leíró listához (\ref{dataflow-path-API-diagram}. ábra).
...
\func{PathElement} struktúrából származik. Nézzük meg részletesebben ezeket a
struktúrákat.
...
értelmében\footnote{szabványrész} mindig egy \func{MoveToElement} kell
legyen. Utána pedig tetszőlegesen sok elem következhet. Jellemzően az
alakzatokat egymás utáni hívások sorozataként építjük fel. Ebből az is
következik, hogy jó pár viszonylag hasonló méretű struktúra memóriába
való lefoglalására kerül sor egy alakzat építésekor. Ugyanakkor érdemes
kerülni az ilyen jellegű foglalásokat. Ezért egy ún. \emph{Region} modellt
alkalmaztunk a lista tárolására.

Ez az osztály (\func{Region}) több a tárolandó struktúrák méretéhez képest
néhány nagyságrenddel nagyobb memóriaterületet foglal le szintén listába fűzve.
A struktúrákat pedig ebbe a Region listába tároljuk úgy, hogy közvetlen
hozzáférünk a már lefoglalt memóriához. Amennyiben az aktuális lefoglalt
részből kilógna az újabb struktúra mérete, akkor a Region osztálytól újabb
szabad területet kérünk. Ez pedig már valóban memóriafoglalást eredményez,
...
kevesebb hívásból valósulhat meg.

Most pedig nézzük át a \func{Path} osztályhoz tartozó függvényeket
(\ref{dataflow-path-API-diagram}. ábra). Vannak egyrészt az elemeket hozzáadó,
...
A bejárás lehetőségét pedig a \func{firstElement} függvény biztosítja. Ettől az
elemtől kezdve végig sétálhatunk a struktúrákat tartalmazó listán. Az
\func{isEmpthy} függvénnyel lekérdezhetjük, hogy üres-e az alakzatot leíró
lista. A \func{lastElement} pedig megadja az utolsó elemet a listában.

A már ismertetett struktúrákhoz mind tartozik egy függvény ami egy
példányt add a listához. Ezek a függvények bizonyos feltételeket
megvizsgálva és a szabványhoz igazodva alkalmaznak bizonyos egyszerűsítéseket.
Az \func{addMoveToElement} függvény például kihasználja azt a
nyilvánvaló tényt, hogy az egymásután következő \func{moveTo} hívások
összevonhatók, és csak a legutolsó hívás koordinátáját kell
letárolni. Minden vonalhúzó struktúrára alkalmazni kell például, hogy ha
elsőként kerülne az üres listába, akkor a végpontját meghatározó
koordinátát használva egy alakzatkezdő \func{MoveToElement} struktúraként
kell értelmezni. De ilyen könnyítő feltétel az is, hogy olyan egyenes
vonalat nem kell a listához adni, aminek a végpontja megegyezik az utolsó
elem végpontjával; vagy azt a másodrendű Bézier-görbét aminek a
kontrollpontja és a végpontja egybeesik, azt egyszerűen egyenes vonalként
adhatjuk a listához. Az \func{addArcElement} függvény esetében már sokkal
komolyabb beállításokat és egyszerűsítéseket hozhatunk. Nyilvánvaló,
hogy egy körciklusnál nagyobb körbejárási szög leegyszerűsíthető. Vagy
egy nulla méretű sugarú kör sem kell egy a középpontba húzott vonalnál
több legyen.

Van egy olyan függvény, amelyikhez nem tartozik külön struktúra. Ez az
\func{addArcToElement}. Itt azért nem volt szükség külön struktúrára,
mert ez a függvény által leírt elem megadható egy egyenes vonal és egy
körcikk segítségével. Ezért ez a függvénytörzs ezt a visszavezetést
tartalmazza.

Az \func{addLineToElement}

 Ha nincs előtte elem, akkor úgy
kell tekinteni rá mint egy \func{MoveToElement}. Itt is
érvényes, hogy amennyiben az előző elem végpontjával megegyezik a végpontja,
akkor összevonható, még pontosabban nem szükséges hozzáadni a listához.
 Ha nincs még elem a listában
vagy az előző elem egy \func{MoveToElement}, akkor figyelmen kívül kell hagyni,
vagyis el kell vetni a hozzáadását.
 Ha nincs előtte elem, akkor szintén
kezdő elemként kell tekinteni a végpontjára. Amennyiben a kontrollpont egyenlő
a végponttal, akkor az elem leegyszerűsödik egy \func{LineToElement}-re.
