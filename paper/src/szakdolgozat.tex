...
megismerkedni.
  ...
  Mit értünk pontosan matematikailag seprő
...
miért lesz hasznos számunkra.
  ...
  A másod- és harmadrendű Bézier-görbék
...
OpenGL rajzoló modullal működjön. Vagyis lehetőséget kell kínálni, hogy a
jövőben több grafikus meghajtóval is tudjuk használni. Mint például a Vulkan,
DirectX, esetleg egy referencia szoftveres raszterizáló. Ezért a Gepard három
részre tagolódik (\ref{structure_eps}. ábra). A \emph{külső} réteg alapja a
Canvas API. Ezt neveztük el \emph{Gepard}-nak. A következő \emph{köztes} réteg
fogja össze a közös részeket. Ez lett a \emph{Gepard Engine}. A \emph{Gepard
Backend} pedig az \emph{alsó} szint, ahol az egyes grafikus meghajtók
specifikus részei kaptak helyet. Három modul fejlesztése történik itt:
\emph{GLES2} (OpenGL ES 2.0), \emph{Vulkan} (Vulkan 1.0) és \emph{Software}
(saját raszterizáló).
  ...
  A felső réteget a \func{Gepard} osztályt \aref{Háttér}.
...
kezelőfelületet adjunk. Érdemes röviden átnézni ezt a felületet.
  ...
  A Canvas interfész két csoportra osztható. Az egyik a hívható függvények, a
másik a beállítható adattagok csoportja. A hívható függvények tekintetében
egyszerűbb a helyzet. Hiszen az interfész által leírt függvények szemantikája
nagyon hasonló a C++ által elvártakhoz. A C++ környezet ugyan kissé megkötőbb.
Például szövegként (\inenglish{string}) átadott számértéket nem fogad el a
szigorú típusosság miatt. Ez azonban nem tekinthető durva megkötésnek, hiszen
ugyanakkor az alap konverziók, átváltások (\inenglish{cast}) továbbra is
működnek.
...
könnyen megadható.
  ...
  A nehezebb részt a másik csoport a módosítható adattagoké jelentette
(\inenglish{members}). Itt a viselkedés kényelmes biztosításához saját
osztályt kellett írni. Ez lett az \func{Attribute} osztály. A bevezetésének
szükségességét a \func{fillStyle} adattagon keresztül mutatom be. A
\func{fillStyle} definíció szerint egy CSS attribútum\footnote{TODO: link
...
a kitöltés színét tárolja. Ugyanakkor a beállítás módja lehet egy hexa-kód
(\func{\#0f0}), egy előre definiált név (\func{green}), esetleg egy komplexebb
-- függvényszerű leírás. Ami azt illeti, ez könnyen feloldható, hiszen egy erre
felkészített kód minden egyes beállításnál egy közös számnégyes formájában
eltárolhatja az értékeket. Igen ám, de érdemes emlékezni, hogy a
\func{fillStyle}-hoz hasonlóan viselkedő attribútumokból még van egy jó pár a
Canvas-ben, amik viszont sokszor nem egy színt, hanem egy tulajdonságot vagy
csak egy sima szám értéket tárolnak. Éppen ezért az \func{Attribute} osztály
minden példányához be tudunk kötni egy a beállításkor meghívódó függvényt
(\inenglish{callback function}), amely a kapott szöveget megpróbálja
értelmezni, beolvasni (\inenglish{parsing}), és amennyiben sikerül, beállítja
az adott adattagot.

  Tehát a \emph{felső} réteg a \func{Gepard} mint interfész egy jól bevált
Canvas API-ra támaszkodik. Ugyanakkor C++ környezetbe ágyazva -- többek között
az erős típusosság miatt is -- szükségessé vált további társ osztályok
definíciója, illetve egy komplexebb adattag kezelő osztály létrehozása.
Viszont ezeken túl jobbára nincs másra szükségünk, hogy megfeleljünk a Canvas
elvárásainak. Így ez a réteg ki is elégíti a feladatunk célkitűzéseit. Vagyis
...
grafikus meghajtó specifikus alsóbb szintű kódokkal, másrészt a közös részeket
valósítja meg. Ez a \func{GepardEngine} osztály. Nézzük át részletesebben az
itt megvalósított részeket.

  Az egyik fontos eleme az alsó réteget elérhetővé tevő
\func{GepardEngineBackend} interfész. Erről majd később részletesen lesz szó.
  A másik fontos osztály a \func{GepardContext}. Ez alapvetően egy tároló
osztály. Itt van elmentve a \func{Surface} is, ahová rajzolunk (ld. később).
Ugyan csak itt tároljuk az alakzatot azaz a path-t is, vagy a már említett
kitöltési színt, stílust, vagy a körvonal paramétereit is. Ami fontos
tulajdonság ebben az osztályban az az, hogy biztosítania kell, hogy a rajzolási
beállítások (paraméter értékek) elmenthetők és visszaállíthatók legyenek. Ezért
ezeket a paramétereket összefogja egy \func{GepardState} nevű osztály. A
\func{GepardContext}-nek mindig van egy (aktuális) státusza, amely tartalmazza
a kurrens beállításokat. Azonban az aktuális állapotot bármikor elmenthetjük a
\func(save()) paranccsal, és bármikor visszaállíthatjuk a \func{restor()}-ral.
A működéi logikája pedig egy verem puffer azaz egy LIFO konténer. Fontos
megjegyezni, hogy néhány \func{GepardContext}-beli elemre nincs hatása. Ilyen
például a path is. Ugyanakkor a rajzoló stílusokat és az affin
transzformációkat el lehet menteni.
...
alfejezete legyen a \emph{Path API} résznek.
    ...
    \begin{wrapfigure}{l}{0.5\textwidth}
    \begin{center}
      \includegraphics[scale=0.5,bb=170 380 365 620,
...
összehasonlítást fogom bemutatni.
  ...
  \begin{wrapfigure}{l}{0.5\textwidth}
  \begin{center}
    \includegraphics[scale=0.2]{img/built/tiger}
  \end{center}
