...
Ez az osztály (\func{Region}) a tárolandó struktúrák méretéhez képest néhány
nagyságrenddel nagyobb memóriaterületet (memóriarégiót) foglal le. A
struktúrákat pedig ebben a memóriarégióban listában tároljuk úgy, hogy
közvetlen hozzáférünk a már lefoglalt memóriához. Amennyiben az aktuális
lefoglalt részből kilógna a következő struktúra mérete, akkor a \func{Region}
osztálytól újabb szabad területet kérünk. Ez pedig már valóban memóriafoglalást
eredményez, ugyanakkor a nagyságrend függvényében jóval kevesebbszer. A modell
másik nagy előnye, hogy az eldobott alakzat memóriahasználatának felszabadítása
is sokkal kevesebb hívásból valósulhat meg. Jelenleg a lefoglalt memóriarégiók
mérete egységesen $2048$ bájt, és előre eldöntött, azaz nem változik.
Természetesen a jövőben, a Gepard optimalizálásának fázisában akár dinamikusan
változó méretű memóriarégiókat is használhatunk; persze ha ennek hasznosságát
...
függvény ami egy példányt add a listához. Ezek a függvények bizonyos
feltételeket megvizsgálva és a szabványhoz igazodva alkalmaznak bizonyos
egyszerűsítéseket. Az \func{addMoveToElement} függvény például kihasználja azt
a nyilvánvaló tényt, hogy az egymásután következő \func{moveTo} hívások
összevonhatók, és csak a legutolsó hívás koordinátáját kell letárolni. Minden
vonalhúzó struktúrára alkalmazni kell például, hogy ha elsőként kerülne az üres
listába, akkor a végpontját meghatározó koordinátát használva egy alakzatkezdő
\func{MoveToElement} struktúraként kell értelmezni. De ilyen könnyítő feltétel
az is, hogy olyan egyenes vonalat nem kell a listához adni, aminek a végpontja
megegyezik az utolsó elem végpontjával; vagy azt a másodrendű Bézier-görbét
aminek a kontrollpontja és a végpontja egybeesik, azt egyszerűen egyenes
vonalként adhatjuk a listához. Az \func{addArcElement} függvény esetében már
sokkal komolyabb egyszerűsítést alkalmazhatunk. Nyilvánvaló, hogy egy
körciklusnál nagyobb körbejárási szög leegyszerűsíthető egy ciklusra. Vagy egy
nulla méretű sugarú kör sem kell egy a középpontba húzott vonalnál több legyen.

Van egy olyan függvény, amelyikhez nem tartozik külön struktúra. Ez az
\func{addArcToElement}. Itt azért nem volt szükség külön struktúrára, mert ez a
függvény által leírt elem megadható egy egyenes vonal és egy körcikk
segítségével. Ezért ez a függvénytörzs ezt a visszavezetést tartalmazza. Arról
már volt szó~\footnote{TODO: ellenőrizni}, hogy ehhez a függvényhez egy olyan
...
könnyedén definiálhatunk egy a sarkainál lekerekített téglalapot. Ha most ezt a
téglalapot vesszük, akkor négy ilyen alakzatelemből felépíthetjük. Ugyanakkor a
valóságban négy egyenes vonalból és négy körcikkből épül fel. Ennél az elemnél
tudjuk, hogy mindig benne van a használt pontok által meghatározott alakzatban,
csupán a sarkoknál a megadott rádiusz(ok) hatására körcikkeket kell beszúrjunk.
A visszavezetés azt kell eldöntse, hogy milyen szögtől milyen szögig terjed a
körcikk, és az előző elem végpontjából hogyan kell vonalat húzni a lekerekítést
adó körcikkhez.
