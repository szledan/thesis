...
%%% Az INCLUDEGRAPHICS ,,resolution'' kulcsszavának definíciója.
% resolution=<res in DPI>
%\makeatletter
%\define@key{Gin}{resolution}{\pdfimageresolution=#1\relax}
%\makeatother

%%% Az INCLUDEGRAPHICS kulcsszavainak globális beállításai.
%\setkeys{Gin}{resolution=2400}
...
%%% A lábjegyzeteket nem töri át másik oldalra
...
C++-os környezetben használható 2D alakzatrajzoló (Path) függvénykönyvtár
készítése. Interfészként egy ismertebb API választása, és az alakzatrajzoláshoz
egyszerű GPU alapú renderelő API használata.
...
OpenGL, és ilyen kíván lenni a \emph{Gepard}\footnote{
{\footnotesize\url{https://github.com/GepardGraphics/gepard/}}} is, ami a
\emph{GL2D}\cite{Herczeg:2014:GL2D} renderelő továbbélése. A cél, hogy
létrehozzunk egy \emph{C++11}-es nyelven írt és több grafikai renderelő API-t
(OpenGL~ES~2.0, Vulkan) támogató függvénykönyvtárat.
...
készített \emph{Canvas 2D Context}~\cite{Cabanier:14:HCC} specifikáció.

A \emph{Canvas 2D Context} (a továbbiakban \emph{Canvas}) specifikáció 2015-ös
kiadása több elkülönített részből állítja össze az API-t. Ezen részek egyike az
úgynevezett \emph{Path}. Az említett 2015-ös specifikáció az 5., 8. és 11.
...
\emph{alakzatokat} (\inenglish{path}) is. \Az+\refstruc{sec:Hatter+ban}
...
Első körben nem volt cél a ,,leggyorsabban futó'' és a ,,legkisebb méretű''
Path-t rajzoló függvénykönyvtár elkészítése. Célunk, egy lehetséges megvalósítás
megtalálása, és ez alapján egy olyan eszköz megalkotása, amely képes a Canvas
specifikáció elvárásait teljesíteni.
...
mint \emph{API} hivatkozunk.} specifikációról, a GLES2 szabvány nyújtotta
...
{\footnotesize\url{https://www.w3.org/TR/2008/WD-html5-20080122/\#the-2d}}}. A
specifikáció leír egy felbontásfüggő rajzolóvásznat, amelyet jellemzően
...
A specifikáción belül definiált Path API egy jól elkülönülő alakzatszerkesztő
...
korlátok. Hiszen józan ésszel belátható, hogy egy egyenes szakaszból definiált
...
ahol a \func{closePath} parancsot meghívtuk (háromszög), a másodikban pedig nem
(V alak). Az első oszlopban a \func{fill}-t, a harmadikban pedig a
\func{stroke}-ot nem hívtuk meg. A specifikáció ugyanakkor rendelkezik arról, hogy
...
Most pedig nézzük meg, hogy milyen konkrét eszközöket biztosít a specifikáció.
Ebben a szakaszban nem térünk ki részletesen a teljes specifikációra. A legfontosabb
...
után térünk rá a specifikáció Path-t definiáló részére.
...
az aktuális transzformációt, és itt van letárolva az aktuálisan
...
Viszont a legtöbb -- a specifikációt megvalósító -- renderelő API támogatja a két
...
A vertex shader-ben az \func{attribute} kulcsszóval különböző bemeneti
...
Hasonlóan a vertex shader-hez, a \emph{fragment shader} esetében is be kell
állítanunk a megfelelő értékeket. A shader-kódban használt \emph{textúrák}
...
hogy hivatkozni tudjunk a textúrákra a shader kódokban, ahhoz azokat is be kell
...
attribútum tömbökbe --, és egyszerre kirajzolni a lehető legtöbbet.
...
a görbe \emph{rangjának}, vagy \emph{fokszámának} nevezzük.\end{defi}
  ...
  \psfrag{P0}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}$}}
  \psfrag{P1}[b][B][0.8]{\bf{$\boldsymbol{p}_{1}$}}
  \psfrag{P2}[b][B][0.8]{\bf{$\boldsymbol{p}_{2}$}}
  \psfrag{P3}[b][B][0.8]{\bf{$\boldsymbol{p}_{3}$}}
  \psfrag{P01}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}^{1}$}}
  \psfrag{P02}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}^{2}$}}
  \psfrag{P11}[b][B][0.8]{\bf{$\boldsymbol{p}_{1}^{1}$}}
  \psfrag{P12}[b][B][0.8]{\bf{$\boldsymbol{p}_{1}^{2}$}}
  \psfrag{P21}[b][B][0.8]{\bf{$\boldsymbol{p}_{2}^{1}$}}
  \psfrag{P03}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}^{3}$}}
  \psfrag{b}[b][B][0.9]{\emph{(b)}}
  \psfrag{Q0}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}$}}
  \psfrag{Q1}[b][B][0.8]{\bf{$\boldsymbol{p}_{1}$}}
  \psfrag{Q2}[b][B][0.8]{\bf{$\boldsymbol{p}_{2}$}}
  \psfrag{Q01}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}^{1}$}}
  \psfrag{Q02}[b][B][0.8]{\bf{$\boldsymbol{p}_{0}^{2}$}}
  \psfrag{Q11}[b][B][0.8]{\bf{$\boldsymbol{p}_{1}^{1}$}}
    \centering
...
a $t=\frac{1}{2}$ értéknél adódik.\footnote{M. Goldapp: i. m., 230. o.}
  ...
  \psfrag{P0}[b][B][0.8]{\bf{$\qquad\boldsymbol{p}_0=\begin{bmatrix} 1 \\ 0 \end{bmatrix}$}}
  \psfrag{P1}[b][B][0.8]{\bf{$\qquad\boldsymbol{p}_1=\begin{bmatrix} 1 \\ h \end{bmatrix}$}}
  \psfrag{P2}[b][B][0.8]{\bf{$\hspace{3cm}\qquad\boldsymbol{p}_2=\begin{bmatrix}\cos\alpha\\ \sin\alpha
                              \end{bmatrix} + h \begin{bmatrix} \sin\alpha \\
                              -\cos\alpha \end{bmatrix}$}}
  \psfrag{P3}[b][B][0.8]{\bf{$\qquad\boldsymbol{p}_3=\begin{bmatrix}\cos\alpha\\ \sin\alpha
                              \end{bmatrix}$}}
    \centering
...
\az{\cite[Goldapp]{Goldapp:1991:approximation}} tanulmányban
...
A felső réteget, a \func{Gepard} osztályt \az+\refstruc{sec:Hatter+ban}
...
lemásoljuk. A cél az, hogy egy a C++ környezetben könnyen használható -- a webes
...
Nézzük meg röviden ezt az osztályt is. Tehát egyrészt, mint már az előbb
említettük, az értékadás sokféleségét biztosítani kell. Ugyanakkor nem csak az
értékadás lehet többféle, hanem az adattagok ,,típusa'' is. Vagyis az említett
\func{fillStyle}-hoz képest más típusú adatot tároló módon is kell tudjon
...
Symbolic Computation, Kluwer Academic Publishers, 2000, 13, 11–49.}}
biztosítja. A bonyolultabb értékadásokat (pl. függvény leírás) pedig egy a
példányosításkor bekötött elemző-értelmező függvény végzi.
...
szakaszában (\ref{sec:A_Canvas_API}) már tárgyaltuk a transzformációkat, itt
...
kitöltés színét. Ezt a színt továbbítja a fragment shader-nek, ami minden olyan
...
eredménypufferbe. Ezzel megjelent a téglalap. Egy ilyen téglalap nem
...
magáról, hogy a speciálisak közül melyik, tehát: \func{MoveTo}, \func{LineTo}
...
az \func{applyTransform} függvénnyel tehetjük meg. Gyakorlatilag a kapott affin
...
leíró \func{MoveToElement} struktúrát. A specifikáció szerint minden alakzat egy
...
Két vonal csatlakozásakor úgy kell eljárni, hogy a hézagokat értelemszerűen
...
akkor kerülnek képbe \az+\refstruc{sec:Hatter+ban} tárgyalt matematikai
visszavezetések.
...
lépéseket.
  \begin{enumerate}
    ...
    \begin{description}[topsep=0ex]
      \item[Ha igen] Megállunk.
    \end{description}
    \item Kivesszük a legfelső görbedarabot, és megvizsgáljuk, hogy leírható-e
    szakaszként (azaz, nem kell tovább felbontani).
    \begin{description}[topsep=0ex]
      \item[Ha igen] A görbedarabot, mint -- végpontokat összekötő -- szakaszt
      beszúrjuk a \func{SegmentTree} struktúrába, és folytatjuk a 3-as ponttal.
      \item[Különben] A görbedarabot a $\boldsymbol{p}^n_0(t=\frac{1}{2})$
      pontjában kettévágjuk, és a két görbedarabot a verembe tesszük.
    \end{description}
    \item Megvizsgáljuk, hogy betelt-e a verem.
    \begin{description}[topsep=0ex,noitemsep]
      \item[Ha igen] A legfelső görbedarabot kivesszük, és azzal újrakezdjük az
      1-es pontban. (Rekurzív rész. Ritkán hívódik.\footnote{A dolgozatban
      később ismertetésre kerülő Ghostscript Tiger.svg renderelésekor mindössze
      egyszer hívodik meg.})
    \end{description}
...
Bézier-görbékkel közelítjük. \Az+\refstruc{sec:arctobezier+ben} bemutatott
...
Az OpenGL~ES~2.0 működését \az+\refstruc{sec:GLES2:API+ban} már bemutattuk. Volt
...
vertexekre fog lefutni a vertex shader-ből készített program.
...
\func{GLfloat} ($4 \times 32 bit$) méretére legyenek az adatok igazítva,
nekünk viszont hét \func{GLfloat} típusú értékünk van.
...
Miután meghatározta a vertex program az aktuális vertex kibővített pontját,
...
lefedettség, akkor az összes ($16$ darab) osztott részt érintjük, és az azokhoz
...
lebegőpontos érték.} állítjuk. Majd eldöntjük, hogy a fragment belső pont-e, és
...
eredményeket néhány ismertebb Canvas 2D Context specifikációt támogató renderelő
...
Ghostscript Tiger.svg példán keresztül vizsgáljuk és mutatjuk
be. Ennek a renderelése teljes. Ami azt jelenti, hogy \emph{minden} elemet
\emph{mérethelyesen} megjelenítünk. A kitöltés esetén a \emph{színhelyesség} is
igaz, ugyanakkor -- más renderelők eredményeivel összevetve -- a széleknél
vannak eltérő pontok. Ennek okaira még kitérünk.
...
nagyított alsó képeken részben az élsimításból adódó különbségeket láthatjuk.
  ...
  \begin{figure}
    \centering
    \newcommand{\tigerviewsize}{%
      \setkeys{Gin}{viewport=345pt 575pt 480pt 660pt,clip,width=\textwidth}
      \presetkeys{Gin}{clip}{}
    }
    \newcommand{\tigersubfigure}[2]{%
      \begin{subfigure}[b]{0.3\textwidth}
        \tigerviewsize \includegraphics{#1} #2
      \end{subfigure}
    }
    \tigersubfigure{img/built/tiger}{}
    ~
    \tigersubfigure{img/tiger-chrome-70_0_3538_77}{}
    ~
    \tigersubfigure{img/tiger-safari-12_0-14606_1_36_1_9}{}

    \renewcommand{\tigerviewsize}{%
      \setkeys{Gin}{viewport=385pt 580pt 420pt 590pt,clip,width=\textwidth}
      \presetkeys{Gin}{clip}{}
    }
    \tigersubfigure{img/built/tiger}{\caption{Gepard}\label{fig:tiger-gepard}}
    ~
    \tigersubfigure{img/tiger-chrome-70_0_3538_77}{\caption{Skia (Chrome)}
      \label{fig:tiger-chrome}}
    ~
    \tigersubfigure{img/tiger-safari-12_0-14606_1_36_1_9}{\caption{WebKit
    (Safari)}
      \label{fig:tiger-firefox}}
    \caption{Kinagyított részek a \emph{Gepard}, a \emph{Skia} és a
    \emph{WebKit} eredményeiből (részletek a szövegben)}\label{fig:tiger-eyes}
  \end{figure}

Az élsimításból adódó különbségek okát abban látjuk, hogy a használt
\emph{élsimító stratégiák} és \emph{finomsági faktorok} nem egyeznek meg. Azaz, a
Gepard és az összehasonlításként használt renderelő API-k, más és más élsimító
megközelítést használ(hat)nak. A Gepardban egy alap módszert használtunk
(pásztázó egyenesek és szubpixelek), a lehető legfinomabb faktorral (16).

A \emph{strukturális hasonlóság} (\inenglish{Structural Similarity} -- SSIM\cite{Zhou:2004}
 %\footnote{Zhou Wang et al.: \emph{{Image
 %quality assessment: from error visibility to structural similarity}, in IEEE
 %Transactions on Image Pro- cessing, 13. évf. (2004. April) 4. sz., 600–612. p.
 %ISSN 1057-7149}}
) kapcsán azt mondhatjuk, hogy a kitöltések helyesek, s így nem hiányzik
alakzat a képről (lásd \az+\emph{\ref{fig:diff}. ábrát}). Ugyanakkor az
élsimítás okán tapasztalt eltérést is jól mutatja az említett ábrán a teljes
eltérés képe. Az ábrán egy részletet kiemeltünk, itt a ,,szakáll'' három pont
vastagságú körvonalának különbségmintázatát látjuk. A szélek különböznek, de a
belső rész azonos.

\Az+\emph{\ref{tab:ssim}. táblázatokon} számszerűsítve is láthatjuk a
strukturális hasonlóságot, illetve a \emph{jel-zaj viszonyt} (\inenglish{Peak
Signal-to-Noise Ratio} -- PSNR\cite{Zhou:2004}
 %\footnote{Zhou Wang et al.: i. m.}
). \Az+\emph{\ref{tab:ssim:bal}
...
kifejező 1-nél.
  ...
  \begin{figure}
    \captionof{table}{Metrikus hasonlóságok}
    \label{tab:ssim}
    \centering
    ...
    \centering
      ...
      \begin{tabular}{|c|c|S[round-mode=places,round-precision=3]
                          ...
                          |S[round-mode=places,round-precision=3]|}
        \hline
          & Metrika & \multicolumn{1}{c|}{Skia}
                    & \multicolumn{1}{c|}{WebKit}
                    & \multicolumn{1}{c|}{Edge} \\
        \hline
        \hline
          \input{../../../paper/src/res/ssim-psnr.txt}
        \hline
      \end{tabular}
    \endminipage
  \end{figure}
...
igaz, hogy a legnagyobb eltérés nem éri el a 0.01-ot. Illetve a jel-zaj viszony
...
kitölteni. A negyedik sor az explicit kitöltést indukáló hívások számát
...
lesznek kirajzolva az alpha textúrába.
  ...
  \begin{table}[!t]
    \captionof{table}{A Ghostscript Tiger.svg rajzolásakor keletkezett
    hívások és elemek számai} \label{tab:numbers}
    \renewcommand*{\arraystretch}{1.2}
    \small
    \centering
    \begin{tabular}{|l|r|}
  ...
  \begin{figure}[!b]
    ...
    {img/built/diff-tiger-chrome-70_0_3538_77-tiger-safari-12_0-14606_1_36_1_9}
      \caption{Skia -- WebKit}
    \end{subfigure}
    \hspace{0.1\textwidth}
    \begin{subfigure}[b]{0.4\textwidth}
      ...
      \bottominset{
        ...
        \fbox{
          \includegraphics[width=0.2\textwidth,viewport=305pt 10pt 315pt
          20pt,clip]{img/built/diffover}
        }
      }
      {
        \includegraphics[width=0.8\textwidth]{img/built/diff}
        \linethickness{1pt}
        \begin{picture}(2,2)
          \put(-99,2){\framebox(5,5){}}
%           \put(-113,7){\line(-15,10){50}}
        \end{picture}
      }{0pt}{-30pt}
      \caption{Gepard -- Skia}
    \end{subfigure}
    \caption{A \emph{Gepard} és a \emph{Skia (Chrome)} által renderelt
    Ghostscript Tiger.svg eredmények különbsége. Az ábrán jól látható,
    hogy a különbségben a körvonal pontok az érintettek, a kitöltés
    azonos.}
    \label{fig:diff}
  \end{figure}
...
Végeredményben azt mondhatjuk, hogy a dolgozatban kitűzött célunkat --
miszerint alkossunk meg egy Canvas 2D API-t használó OpenGL ES 2.0-án alapuló
2D alakzatrajzoló C++ nyelvű programot -- elértük.
\Az+\emph{\ref{tab:functions} táblázatban} még feltüntettük a megvalósított, a
rajzolás szempontjából elengedhetetlen funkcionalitások biztosító függvényeket.
Illetve a táblázat második oszlopában a rajzolást effektív nem gátoló, de a
Canvas 2D specifikációban szereplő függvényeket is. Ezeknek a megvalósítása egy
jövőbeli munka része. Mint ahogy az is, hogy a jelenleg helyes eredményt nyújtó
renderelő egységeket \emph{futásidő} és \emph{memóriahasználat} szempontjából
javítsuk.
...
alakzatrajzoló API, ami megvalósítja a Canvas előírásait, létrehozva egy GPU
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  \nocite{Strachey2000}
  %\nocite{Zhou:2004}
  %\nocite{Hughes:2013:CGP}
  %\notice{Munshi:2008:OEP:1481069}
  %\notice{Kurusa:1999:szamitogepes}
...
\fancyhead[L]{\MakeUppercase{\thischaptertitle}}

  \begin{figure}[!htb]
    \begin{center}
      \includegraphics[scale=0.45]{img/built/tiger}
    \end{center}
    \caption{\label{appendix:tiger} A számítógépes grafikában gyakran használt
    \func{Ghostscript Tiger.svg} képe Geparddal renderelve}
  \end{figure}
  ...
  \begin{figure}[!htb]
    ...
    \end{center}
    \caption{\label{appendix:dataflow} A rajzolás folyamatai}
  \end{figure}
