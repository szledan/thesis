...
\emph{alakzatokat} (\inenglish{path}) is. \Aref{sec:Háttér}. fejezetben
...
Ebben a fejezetben bemutatjuk azokat a technológiákat és kihívásokat, amelyek
keretet szabnak a téma megvalósításának. Szót ejtünk magáról a Canvas 2D
Context API\footnote{Az \emph{alkalmazásprogramozási felület/interfész}
(\inenglish{Application Programming Interface}) kifejezésre mint \emph{API}
hivatkozunk.} előírásról, a GLES2 szabvány nyújtotta lehetőségekről és azokról
a matematikai módszerekről, amelyek biztosítják számunkra, hogy ,,tetszőlegesen
bonyolult'' alakzatokat helyesen megjeleníthessünk. Össze kell kapcsolni a
GLES2 szabta korlátokat\footnote {Pl. a GLES2 legfeljebb háromszögeket rajzol.}
a Canvas elvárásaival. Ami azt jelenti, hogy fedjünk le háromszögekkel olyan --
akár önmetsző -- alakzatokat, amelyeket tetszőleges számú szakasz, Bézier-görbe
és körcikk alkot, s mindezeket mérethelyesen jelenítésük is meg. Ehhez -- mint
látni fogjuk -- több lépésbeli megközelítést (\inenglish{approximation})
alkalmazunk.
...
\emph{RGBA} (\emph{vörös, zöld, kék, opacitás}) vagy az \emph{HSV}
...
fejezet.}: \[\begin{pmatrix}a & c & e \\ b & d & f \\ 0 & 0 & 1 \\
\end{pmatrix}\]
...
\emph{vertex attribútumtömbnek} nevezzük. A vertex shaderben az
...
vertex programot leíró shader-kódban. Ezeknek az attribútumoknak a közös
méretét \emph{lépéshossznak} (\inenglish{stride length}) nevezzük, és minden
...
is két lehetőség közül választhatunk. Egyrészt megadhatjuk az attribútumok
bekötésekor. Másrészt egy egészen korai szakaszban készíthetünk egy viszonylag
nagy (akár $65536$ elemű) indextömböt, és ezt feltölthetjük a GPU oldalra. Ha
pedig a renderelés pillanatában\footnote{A renderelést a \func{glDrawElements}
vagy a \func{glDrawArrays} függvények hívása jelenti.} nem adunk meg
indextömböt, akkor sorra ebből a feltöltött pufferből veszi az indexeket. Ez az
utóbbi megoldás akkor lehet hasznos, ha nagyon sok hasonló geometriai elemet
(pl. trapézokat) szeretnénk ismétlődő attribútumokkal kirajzolni. Ilyenkor az
indextömböt könnyedén generálhatjuk, és többször is felhasználhatjuk.
...
a következőig minden pontot jelölünk, majd megint nem jelöljük a rákövetkezőig,
és így tovább. Egy pont az alakzat része, ha meg van jelölve. A megjelölt
pontok összességének képe a kitöltött alakzatok képe.
...
tekinthetők, mint az előbbi geometriai félegyenesek.\footnote{Azzal a
...
%adott az ($n+1$)-darab $p_0,...,p_n$ pont. Ezeket Bézier, vagy kontrollpontoknak, az általuk meghatározott poligont pedig kontrollpoligonnak nevezzük.
...
%$p^n_0(t)$ pont leír, ahogy a $t$ végigfut a $[0,1]$ intervallumon,
%Bézier-görbének nevezzük, és $B(t)$-vel jelöljük. A $p^k_i(t)$ pontokat de Casteljau-pontoknak, az $n$ számot pedig a görbe rangjának hívjuk. \end{defi}
...
A felső réteget a \func{Gepard} osztályt \aref{sec:Háttér}. fejezetben tárgyalt
...
akkor kerülnek képbe \aref{sec:Háttér}. fejezetben tárgyalt matematikai
visszavezetések.
...
végre. Nézzük meg az egyes lépéseket.
  \begin{enumerate}
    ...
    \begin{description}[topsep=0ex,noitemsep]
      ...
      rész. Ritkán hívódik.\footnote{A dolgozatban később ismertetésre
      kerülő Ghostscript Tiger.svg renderelésekor mindössze egyszer
      hívodik meg.})
...
Bézier-görbékkel közelítjük. \Aref{sec:arctobezier}. alfejezetben bemutatott
...
Az OpenGL~ES~2.0 működését \aref{sec:GLES2:API}. alfejezetben már bemutattuk. Volt
...
\emph{élsimító stratégiák} és \emph{finomsági faktorok} nem egyeznek. Azaz, a
...
(pásztázó egyenesek és subpixel-ek), a lehető legfinomabb faktorral (16).

A \emph{strukturális hasonlóság} (SSIM\footnote{Zhou Wang et al.: \emph{{Image
...
ISSN 1057-7149}}) kapcsán azt mondhatjuk, hogy a kitöltések helyesek, s így nem
hiányzik alakzat a képről (lásd \az+\emph{\ref{fig:diff}. ábrát}). Ugyanakkor az
 ...
 %   \begin{threeparttable}
    \minipage{0.3\textwidth}
      ...
      \begin{tabular}{|c|S[round-mode=places, round-precision=3]|}
        \hline
        Színcsatorna & \multicolumn{1}{|c|}{SSIM %\tnote{1}\,
        érték}\\
        \hline
        \hline
...
lesznek kirajzolva az alpha-textúrába.
  ...
  \begin{table}[!t]
    ...
    \begin{tabular}{|l|r|}
      ...
      Ebből körvonal (\func{stroke}) hívás (is) & 524\\
      \hline
      ...
      Vertikálisan összevont trapézok & 197999 \\
      \hline
    \end{tabular}
  \end{table}

\vspace{3cm} % TODO FIXME : REMOVE THISREMOVE THISREMOVE THISREMOVE THIS REMOVE  THISREMOVE THIS
  \begin{figure}[!htb]
    \centering
