...
Ez a függvény feltölti grafikus egységre a GLES2 beállításait és a bekötött
adatokat, és elindítja\footnote{Az, hogy ténylegesen az adott függvénynél
indul-e el a renderelés, azt a GLES2 szabvány nem garantálja. Azt a grafikus
megható gyártójára bízza.} a renderelést. A függvény három paramétert vár. Az
...
trapézonként hatot. Az utolsó paraméter egy segítség a GLES2 API-nak. Ezzel a
paraméterrel az jelezhetjük, hogy kisebb (\func{GL\_UNSIGNED\_BYTE}) vagy
nagyobb (\func{GL\_UNSIGNED\_SHORT}) méretű indextömböt használunk-e. A
trapézok lehetséges magas száma miatt mi rendre a nagyobb méretűt választjuk
majd.
...
iniciatív részben már beállíthatjuk. Az indextömb első hat elem a következő: $0, 1, 2,
2, 1, 3$. Ezek rendre egy-egy trapéz sarokindexei is: bal felső ($0$), bal alsó
($1$), jobb felső ($2$) és jobb alsó ($3$). Az indextömb következő hat indexét
pedig úgy generáljuk, hogy minden számhoz négyet hozzáadunk. Majd a következő
hatot, és így tovább egy maximum $65536$ elemből álló tömböt generálunk. Ezt a
tömböt feltöltjük a GPU memóriájába, és megjelöljük mint az általános
indextömb. Az említett indexszámok jelentése pedig értelemszerűen az egymást
követő trapézokat lefedő két háromszög csúcspontjait rendeli az adott trapéz
sarkaihoz. Egy trapéz sarkához pedig mindig tartozik valódi koordinátapár.
...
állítanunk egy olyan attribútum tömböt, ami a trapézok sarokpontjainak adatait
tartalmazza. Egy csúcsponthoz azonban nem csak egy koordinátapár tartozik.
Ennek az okára majd még visszatérünk. Minden csúcsponthoz (vertexhez) át kell
adnunk a teljes trapézt leíró hat értéket, és a trapézon belüli sarokindexet.
Nézzük meg röviden, hogy mit is értünk mindezen.

Tehát egy trapézt meghatároz négy $x$ és két $y$ érték: $\{x_{l,t}, x_{r,t},
...
háromszög renderelését jelenti. Egy háromszög renderelése három vertex
(csúcspont) kiszámítását jelenti. Ezekre a vertexekre fog lefutni a már
említett vertex shaderből készített program. Minden vertexre külön-külön (de
párhuzamosan) lefut a vertex program úgy, hogy a vertxehez speciálisan kap a
program bemeneti adatokat. A bal felső sarok például a teljes trapézt leíró hat
értéket és a $0$-ás sarokindexet kapja meg. A bal alsó pedig szintén a teljes
trapéz leírást és az $1$-es sarokindexet, és így tovább. Amennyiben csak
simán a két háromszöget renderelnénk ki, nem volna szükség az összes
adatra, elég volna a vertexhez tartozó koordinátát átadni. Viszont a mi
esetünkben az élsimítást is a shader programokban számoljuk ki, ezért az
minden vertexnek -- mint a későbbi leírásban látni fogjuk -- tudnia
kell, hogy melyik sarokpontja a trapéznak.
