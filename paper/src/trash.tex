...
az értéket. Az általunk tervezett API azonban csak a Canvas 2D
    ...
    \includegraphics[scale=0.6]{img/built/w3c_erdely_eps}
...
Contextre koncentrál, így a CSS elemeket nem kívánjuk támogatni.
    ...
    \begin{center}
      %\includegraphics[scale=0.6]{img/built/dataflow_canvas_api_eps}
    \end{center}
...
tagolódik. A \emph{külső} réteg alpaja a Canvas API. Ezt neveztük el
...
\emph{Vulkan} (Vulkan 1.0) és \emph{Software} (saját raszterizáló).
  ...
  A felső réteget (Gepard) \aref{Háttér}. fejezetben tárgyalt Canvas
határozza meg. Természetesen -- mint már említettem -- a Canvas egy alapvetően
webes interfészként fogható fel. Ez azt is jelenti, hogy a Gepard mint
interfész nem teljesen úgy fedi le mintha webes környezetbe lenne ágyazva. A
cél nem is az volt, hogy pontosan lemásoljuk. A cél egy a c++ környezetben
könnyen használható -- a webes szcénában már jól kipróbált -- kezelőfelületet
adjunk.

  Érdemes röviden átnézni ezt a felületet.
...
konverziók, átváltások (\inenglish{cast}) továbbra is működnek.
  ...
  A függvények döntő része 32 bites lebegőpontos (\inenglish{float})
vagy egész (\inenglish{int}) számot vár paraméterként. Ezért ezek könnyedén
kezelhetők. Ugyanakkor pár metódus esetében szükségszerű bevezetni saját
típusokat, osztályokat, struktúrákat, hogy a Canvashoz hasonló viselkedést
biztosíthassuk. Ilyenek például: \emph{Image} a \emph{drawImage}(),
\emph{putImageData}(), stb esetén; vagy az \emph{Element} a
\emph{drawFocusIfNeeded}() használatakor. Szerencsére ezek a struktúrák is jól
...
lett az \emph{Attribute} osztály. A bevezetésének szükségességét a
\emph{fillStyle} adattagon keresztül mutatom be. A fillStyle definíció szerint
...
lehet egy hexakód (\inenglish{\#0f0}), egy előre definiált név
(\inenglish{green}), esetleg egy komplexebb -- függvényszerű leírás
(\inenglish{rgba(0, 255, 0)}). Ami azt illeti, ez könnyen felodható, hiszen egy erre felkészített kód minden
egyes beállításnál egy közös számnégyes formájában eltárolhatja az
értékeket. Igen ám, de érdemes emlékezni, hogy a fillStylehoz hasonlóan
viselkedő attribútumokból még van egy jó pár a Canvasban, amik viszont
sokszor nem egy színt, hanem egy tulajdonságot vagy csak egy sima szám
értéket tárolnak. Éppen ezért az Attribute osztály minden példányához
be tudunk kötni egy a beállításkor meghívódó függvényt
(\inenglish{callback function}), amely a kapott szöveget megpróbálja
értelmezni, és amennyiben sikerül, beállítja az adott adattagot.

  Röviden összefoglalva az ún. \emph{felső} rétegről mondottakat.

  A Gepard mint interfész egy jól bevált Canvas API-ra támaszkodik.
...
definícióját, illetve egy komplexebb adattag kezelő osztály
létrehozását. Viszont ezeken túl jobbára nincs másra szükségünk, hogy
megfeleljünk a Canvas elvárásainak, ami ezen a réteg ki is elégíti a
feladatunk célkitűzésit. Vagyis kapcsolatot teremt a felhasználó és a
következő ún. \emph{köztes} réteg között.


--- Innentől régi szöveg, csak megtartom ---
